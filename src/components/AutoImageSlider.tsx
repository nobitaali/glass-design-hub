...
const AutoImageSlider = () => {
  ...
  if (!isLoaded || products.length === 0) {
    return (
      <div className="relative w-full h-[60vh] md:h-[60vh] max-h-[600px] min-h-[300px] md:min-h-[400px]">
  ...
  return (
    <div className=' relative h-[50vh]  md:h-[60vh] max-h-[600px] min-h-[300px] md:min-h-[400px] flex align-center overflow-hidden justify-space-between flex-col gap-7'>
      <div
      ...
      >
        ...
        <div className="relative w-full h-full">
          ...
          <div className="absolute inset-0 flex items-center justify-start p-4 md:p-8 lg:p-16">
            <div className="text-white max-w-xl md:max-w-2xl">
              <h1 className="text-2xl md:text-3xl lg:text-4xl font-bold mb-2 md:mb-4 leading-tight">
                {currentProduct?.title}
              </h1>
              <p className="text-sm md:text-lg lg:text-xl mb-4 md:mb-6 text-white/90 line-clamp-2 md:line-clamp-none">
                {currentProduct?.description}
              </p>
              <div className="flex w-1/2 flex-col sm:flex-row gap-2 sm:gap-4">
        ...
        <button
          ...
          className="absolute left-2 md:left-4 md:bottom-auto bottom-0.5 md:top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 backdrop-blur-sm text-white p-1 md:p-3 rounded-full transition-all z-10 group"
        ...
        <button
          onClick={goToNext}
          className=" absolute right-2 md:bottom-auto bottom-0.5 md:top-1/2 -translate-y-1/2 bg-white/20 hover:bg-white/40 backdrop-blur-sm text-white p-1 md:p-3 rounded-full transition-all z-10 group"
     ...
     <div className="absolute  max-w-84 overflow-auto bottom-87 sm:bottom-4 left-1/2 -translate-x-1/2 flex gap-1 md:gap-1 z-10 p-2">
  {products.map((_, index) => (
    <button
      key={`dot-${index}`}
      onClick={() => goToSlide(index)}
      className={`w-2 h-2 md:w-4 md:h-4 rounded-full transition-all duration-200  ${
        index === currentIndex
          ? 'bg-blue-600 scale-125 shadow-lg'
          : 'bg-white hover:bg-white/30'
      }`}
